...
Usage:
    ...
    python benchmark_sift1m.py --no_boundary      # disable boundary links (ablation)
"""
...
def recall_at_k(preds, gt, k=10) -> float:
    hits = sum(
        len(set(p[:k]) & set(g[:100]))  # your top-k vs all 100 true neighbors
...
def main():
    ...
    parser.add_argument("--no_boundary", action="store_true",
                        help="Disable boundary links (ablation comparison)")
    ...
    # ── boundary links ────────────────────────────────────────────────────────
    if args.no_boundary:
        print(f"\n[2b] Boundary links DISABLED (ablation mode)")
        engine._boundary_links = {}
    else:
        bl_count = sum(len(v) for v in engine._boundary_links.values()) \
                   if engine._boundary_links else 0
        print(f"\n[2b] Boundary links ENABLED  ({bl_count:,} links loaded)")
    ...
    new_policy = engine.calibrate_gap_policy(queries_norm,
                                             probe_tiers=PROBE_TIERS_1M)
    print(f"  Policy: {[(f'{t:.3f}', p) for t, p in new_policy]}")

    # ── ground truth (cosine) ─────────────────────────────────────────────────
    print(f"\n[4] Loading official SIFT1M ground truth...")
    ground_truth_raw = np.load(data_dir / 'sift1m_groundtruth.npy')
    ground_truth     = ground_truth_raw[query_indices]  # align with sampled queries
    print(f"  GT shape: {ground_truth.shape}")

    # ── benchmark ─────────────────────────────────────────────────────────────
    boundary_tag = "no-boundary" if args.no_boundary else "with-boundary"
    print(f"\n[5] Benchmark  ({args.n_queries} queries, k={GT_K}, {boundary_tag})")
    print(f"{'─'*72}")
    ...
    print(f"\n[6] Multithreaded search  (probe=32, {boundary_tag})")
